#REMOVE BEFORE PR
#test comparing the implementation of unet_diffusion.py and an identity model (forward(x) = x)

#TODO:
#-implement testing and comparison of both thru dummy dataset?
...
class IdentityModel(ModelABC, nn.Module):
    ...
    def __init__(
        ...
        settings: IdentityModelSettings = IdentityModelSettings,
        *args,
        **kwargs,
    ):
...
class UnetDiffusion(ModelABC, nn.Module):
    ...
    def __init__(
        ...
        out_channels: int,
        input_shape: Union[None, Tuple[int, int]] = None,
        settings: UnetDiffusionSettings = UnetDiffusionSettings,
        *args,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
